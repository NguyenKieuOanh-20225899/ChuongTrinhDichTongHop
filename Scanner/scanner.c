...
Token* getToken(void)
{
  ...
  case 2:
    //TODO (Skip blanks)
    while(currentChar == ' ' || currentChar == '\n' || currentChar == '\t' || currentChar == '\v' || currentChar == '\f'){
      readChar();
    }
    state = 0;
    return getToken();
  case 3:
    ...
    int id3 = 0;
    while(currentChar == '_' || currentChar >= 'a' && currentChar <= 'z' || currentChar >= 'A' && currentChar <= 'Z' || currentChar >= '0' && currentChar <= '9'){
      str[id3++] = currentChar;
      readChar();
    }
    str[id3] = '\0'; // Neu check str[0] thuoc 0 - 9 thi tra false
    state = 4;
    return getToken();
  case 4:
  	{TokenType type = checkKeyword(str);
  	if (type == TK_NONE) state=5; else state =6;
    return getToken();}
  case 5:
  	//TODO Identifiers
  ...
  case 7:
    ...
    state = 8;
    return getToken();
  case 8: // ban dau code khong co case 8 ??
  ...
  case 24:
    ...
    cn = colNo;
    readChar();
    if(charCodes[currentChar] == SB_RPAR) state = 25; else state = 26;
    return getToken();
  case 25:
    //TODO
    readChar();
