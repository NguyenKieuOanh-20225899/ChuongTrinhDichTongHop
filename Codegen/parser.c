...
Type* compileLValue(void) {
  ...
  case OBJ_VARIABLE:
    ...
    while (currentScope != NULL && currentScope != var->varAttrs->scope) {
      level++;
...
void compileIfSt(void) {
  ...
  // If with one condition: <condition>, FJ L, <statement>, L:...
  // If with else: <condition>, FJ L1, <then-statement>, J L2, L1: <else-statement>, L2:...
  Instruction* fjInst;
  Instruction* jInst;
  ...
  // Generate FJ instruction (jump if false)
  ...
  if (lookAhead->tokenType == KW_ELSE) {
    // Generate J instruction to skip else part
    jInst = genJ(DC_VALUE);
    ...
    // Update FJ to jump to end (no else part)
    label1 = getCurrentCodeAddress();
    updateFJ(fjInst, label1);
  }
}
...
void compileWhileSt(void) {
  ...
  // L1: <condition>, FJ L2, <statement>, J L1, L2:...
  ...
  // Generate FJ instruction (jump if false to end of loop)
  fjInst = genFJ(DC_VALUE);
  
  eat(KW_DO);
  compileStatement();
  
  // Generate J instruction to jump back to beginning of loop
  genJ(beginLoop);
  
  // Update FJ to jump to end of loop
  endLoop = getCurrentCodeAddress();
  updateFJ(fjInst, endLoop);
}

void compileForSt(void) {
  // Generate code for for-statement
  // For v := exp1 to exp2 do statement
  // Sinh mã theo slide 202:
  // <address of v>, CV, <exp1>, ST (khởi tạo v = exp1, địa chỉ v còn trên stack)
  // L1: CV, LI, <exp2>, LE, FJ L2 (dùng CV để copy địa chỉ v từ stack)
  // <statement>
  // CV, CV, LI, LC 1, AD, ST (tăng v, dùng CV)
  // J L1
  // L2: DCT 1
  Type* varType;
  Type *type;
  ...
  // Calculate level and offset for loop variable
  if (loopVar->kind == OBJ_VARIABLE) {
    level = 0;
    currentScope = symtab->currentScope;
    while (currentScope != NULL && currentScope != loopVar->varAttrs->scope) {
      level++;
      currentScope = currentScope->outer;
    }
    varOffset = loopVar->varAttrs->localOffset;
    varType = loopVar->varAttrs->type;
  } else {
    error(ERR_INVALID_LVALUE, currentToken->lineNo, currentToken->colNo);
  }

  // Initialize loop variable: v := exp1
  // Push address of v and keep it on stack
  genLA(level, varOffset);
  // Copy address for ST (address still remains on stack after ST)
  genCV();
  ...
  // Store initial value: ST (address of v still on stack)
  genST();

  // Mark beginning of loop (before CV, LI)
  beginLoop = getCurrentCodeAddress();

  // Check condition: v <= exp2
  // Copy address from stack
  genCV();
  ...
  // Compile upper bound expression
  type = compileExpression();
  checkTypeEquality(varType, type);
  
  // Check if loop variable <= upper bound
  genLE();
  ...
  // Increment loop variable: v := v + 1
  // Copy address twice (one for ST, one to keep on stack)
  genCV();
  genCV();
  // Load current value
  genLI();
  ...
  genLC(1);
  genAD();
  // Store back (address still on stack)
  genST();
  
  // Jump back to beginning of loop (will CV, LI again at beginLoop)
  genJ(beginLoop);
  ...
  // Clean up stack (remove the address)
  genDCT(1);
}
...
void compileCondition(void) {
  // Generate code for condition with comparison operators
  Type* type1;
  Type* type2;
  TokenType op;
...
Type* compileFactor(void) {
  ...
  case TK_NUMBER:
    eat(TK_NUMBER);
    // Generate code: Load constant onto stack
    genLC(currentToken->value);
    type = intType;
    break;
  case TK_CHAR:
    eat(TK_CHAR);
    // Generate code: Load character constant onto stack
  ...
  case TK_IDENT:
    ...
    switch (obj->kind) {
    case OBJ_CONSTANT:
      // Generate code: Load constant value onto stack
	...
	genHL();
        ...
        // Generate code: Load variable value onto stack
