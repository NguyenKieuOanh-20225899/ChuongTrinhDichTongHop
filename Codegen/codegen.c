...
void genVariableAddress(Object* var) {
  // Generate code to push the address of a variable onto the stack
  // LA level, offset: t := t + 1; s[t] := base(level) + offset;
  int level = 0;
  Scope* currentScope = symtab->currentScope;
  Scope* varScope = VARIABLE_SCOPE(var);
  
  // Calculate the level difference between current scope and variable scope
  while (currentScope != NULL && currentScope != varScope) {
    level++;
...
void genVariableValue(Object* var) {
  // Generate code to push the value of a variable onto the stack
  // LV level, offset: t := t + 1; s[t] := s[base(level) + offset];
  int level = 0;
  Scope* currentScope = symtab->currentScope;
  Scope* varScope = VARIABLE_SCOPE(var);
