...
// CẦN HOÀN THIỆN TRONG TYPE CHECKING BUỔI 1
// Kiểm tra khi khai báo hằng, và factor để in ra những lỗi nào?
...
// CẦN HOÀN THIỆN TRONG TYPE CHECKING BUỔI 2
void compileArgument(Object* param) {
  ...
  // Nếu tham số hình thức là tham biến (PARAM_REFERENCE)
  if (param->paramAttrs->kind == PARAM_REFERENCE) {
    // Tham số thực phải là LValue (biến, tham số, hoặc hàm)
    ...
    // Tham số hình thức là tham trị (PARAM_VALUE)
    argType = compileExpression();
  }
  
  // Kiểm tra sự tương đương kiểu
...
void compileArguments(ObjectNode* paramList) {
  ...
  case SB_LPAR:
    ...
    // Compile first argument if exists
    if (node == NULL) {
      error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
    } else {
    ...
    // Kiểm tra còn tham số hình thức thừa không
    if (node != NULL) {
      error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
    }
  ...
  case KW_ELSE:
  case KW_THEN:
    // Trường hợp không có cặp ngoặc đơn ()
    // Danh sách tham số phải rỗng
    if (paramList != NULL) {
...
// Dùng sai số chiều của mảng so với khai báo
// - 2 trường hợp khai ít chiều dùng nhiều chiều và khai ....
// 